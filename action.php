...
if(isset($_POST["action"]))
{
 if($_POST["action"] == "fetch")
 {
   ...
   foreach($folder as $name)
   {
    $output .= '
     <tr>
      ...
      <td><button type="button" name="update" data-name="'.$name.'" class="update btn btn-warning btn-xs">Actualizar</button></td>
      <td><button type="button" name="delete" data-name="'.$name.'" class="delete btn btn-danger btn-xs">Eliminar</button></td>
      <td><button type="button" name="upload" data-name="'.$name.'" class="upload btn btn-info btn-xs">Subir archivo</button></td>
      <td><button type="button" name="view_files" data-name="'.$name.'" class="view_files btn btn-default btn-xs">Ver archivo</button></td>
     </tr>';
   }
  }
  else
  {
   $output .= '
    <tr>
     <td colspan="6">No se encontró ninguna categoria</td>
    </tr>
   ';
  }
  $output .= '</table>';
 ...
 if($_POST["action"] == "change")
 {
  if(!file_exists($_POST["folder_name"]))
  {
   ...
   echo 'Cambio de nombre de la categoria';
  }
  else
  {
   echo 'Categoria ya creada';
  }
 }
 ...
 if($_POST["action"] == "change_file_name")
 {
  ...
  {
   echo 'Cambio de nombre de archivo exitoso';
  }
  else
  {
 ...
 if ($_POST["action"] == "download_file") {
    $file_path = $_POST["folder_name"] . '/' . $_POST["file_name"];
    if (file_exists($file_path) && pathinfo($file_path, PATHINFO_EXTENSION) === 'pdf') {
        ...
        echo 'Archivo no encontrado o no es un formato PDF';
    }
}
